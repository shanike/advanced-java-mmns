...
public class Polynom {
	 ...
	 * @param coefficients
	 * @param exponents
	 * @throws Exception if coefficient or exponents array are invalid: 1) If the
	 *                   arrays are not the same length. 2) If there's a non-natural
	 *                   integer in {@code exponents}
	 */
	...
	public String toString() {
		...
		for (int i = 0; i < monoms.size(); i++) {
			...
			if (i != 0 && monom.coefficient >= 0) {
				/*
				 * The coefficient is a positive number in the middle of the polynom: render a
				 * preceding "+".
				 */
				finalStr += "+";
			}
			...
			if (Math.abs(monom.coefficient) != 1 || monom.exponent == 0) {
				/*
				 * The coefficient is **not** 1 and **not** -1, OR it **is** 1 or -1 **but** the
				 * exponent is 0: render the coefficient.
				 */
				finalStr += monom.coefficient;
			} else if (monom.coefficient == -1) {
				/**
				 * The coefficient is -1, (and the exponent is **not** 0) and even though it's
				 * negative: render the "-" because we don't render the coefficient itself when
				 * it's -1
				 */
				finalStr += "-";
			}
			...
			if (monom.exponent != 0 && monom.coefficient != 0) {
				/*
				 * The exponent is **not** 0: render the "x" (/and the exponent/).
				 */
				finalStr += "x";

				if (monom.exponent != 1) {
					/*
					 * If the exponent is **not** 1: render the exponent.
					 */
